...
class PlayerCollection:   
   ...
   def Current(self):
     return self.players[self.position_]
   def NextPlayer(self):     
     if self.position_==len(self.players)-1:
       self.position_=0
     else:
       self.position_+=1
     return self.Current()
...
class CommonCore:
    ...
    def GetActivePlayer(self):
        PC=PlayerCollection(self.players)
        
        while True:  
            if PC.NextPlayer().IsActive==True:
                 activeplayer_=PC.Current()
                 PC.Current().IsActive=False
                 PC.NextPlayer().IsActive=True
